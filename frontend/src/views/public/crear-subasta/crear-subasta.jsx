...
import ScrollFaltantes from "../../../components/ui/scroll-figuritas/scroll-faltantes.jsx"; 
...
const PasoCondiciones = ({
  ...
  const toggleFigurita = (fig) => {
    ...
    onCambiarFiguritas(
      existe
        ? figuritasDeseadas.filter((f) => f.id !== fig.id)
        : [...figuritasDeseadas, fig],
    );
  };
      ...
      <div className="px-3 py-1">
        {[
          ...
          },
          {
            label: "Duración",
            valor: duracion
              ? <span className="fw-semibold">{duracion} {duracion === 1 ? "hora" : "horas"}</span>
          ...
          },
          {
            ...
            valor: figuritasDeseadas.length > 0
              ? <span className="fw-semibold">{figuritasDeseadas.map((f) => f.jugador).join(", ")}</span>
              : <span className="text-muted">Sin restricción</span>,
          },
          {
            label: "Calificación mínima",
            valor: calificacionMin === 1
